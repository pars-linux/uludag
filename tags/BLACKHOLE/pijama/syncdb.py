...
class WUpdate:
	...
	def tableactions(self, pckgname, reponame, dirs, root, actionfilelen, numofpatchs, summary):
		...
		try:#try whether there exists a package at the repo, if so just edit the repo date edit info
			...
			p=t.objects.get(pkgname=pckgname, reponame=reponame)# if there is not an entry will throw exception
			...
			## i should check whether there are changes at the package info like new patches, runtimedeps ...
			...
			if not self.spec.source.isA: ISA=None
			else: ISA=self.spec.source.isA[0]
			if not self.spec.source.partOf: PARTOF=None
			else: PARTOF=self.spec.source.partOf
			...
			for pkg in li:
				...
				print pkg.version, pkg.versionFrom, pkg.versionTo
				...
				p.builddeps_set.create(name=pkg.package, version=version, sign=sign, reponame=reponame)
				
				print ("Builddeps done")
				
			info=self.spec.source.packager
			p.packager_set.create(name=info.name, email=info.email, reponame=reponame)
			...
			for patch in patches:
				...
				p.patch_set.create(name=patch.filename, compressiontype=patch.compressionType, level=patch.level, reponame=reponame)
			...
			for pkgknowledge in binaries:			
				for conflict in pkgknowledge.conflicts:
					...
					p.conflicts_set.create(name=conflict.package, version=version, sign=sign, reponame=reponame)
				...
				for dep in runtimedeps:
					...
					p.runtimedeps_set.create(name=dep.package, version=version, sign=sign, reponame=reponame)
			...
			if "screenshots" in dirs:
				for filename in os.listdir("screenshots"):
					p.screenshot_set.create(filename=filename, reponame=reponame)
