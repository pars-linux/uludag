...
from yali4.gui.GUIAdditional import ResizeWidget, LvmWidget
...
class DiskList(QtGui.QWidget):
    ...
    def __init__(self, *args):
        ...
        #self.hbox.addWidget(self.manageLvm)
        
        #self.hbox.addWidget(self.manageRaid)
        #self.hbox.addSpacerItem(spacerItem)
    ...
    def manageLvmPartitions(self):
        pvs = []
        for dev in self.devs:
            pvs.extend(dev.getLVMPartitions())
             
        lvm = LvmWidget(self, pvs, partitionTypes)
        lvm.show()
    
    def manageRaidPartitions(self):
        pass
    ...
    def formatTypeChanged(self, cur=None):
        ...
        #if partitionTypes[cur] == parttype.lvm:
        #    forceToFormat()
        #if partitionTypes[cur] == parttype.lvm:
        #    pass
    ...
    def addDevice(self, dev):
        ...
        diskItem = DiskItem("%s - %s" % (devstr,size_str),dev.getModel(),self.partEdit,dev.getTotalMB(), self)
    ...
    def slotApplyPartitionChanges(self):
        ...
        if hasReq:
            #Raid or Lvm parts more than one
            if not ((not hasReq._partition.getPath() == partition.getPath()) and ((partition.isRaid() or partition.isLvm()))):
...
class DiskItem(QtGui.QWidget):
    ...
    def __init__(self, name, model, partEdit, totalSize, parent):
        QtGui.QWidget.__init__(self,None)
        self.setAutoFillBackground(False)
        ...
        self.manageLvm = QtGui.QPushButton(_("Manage LVM"),self)
        self.manageRaid = QtGui.QPushButton(_("Manage RAID"),self)
        self.hboxlayout.addWidget(self.manageLvm)
        self.hboxlayout.addWidget(self.manageRaid)
        self.hboxlayout.addItem(spacerItem1)
        self.hboxlayout.addWidget(self.deleteAllPartitions)
        #self.vboxlayout.addLayout(self.hboxlayout)

        self.diskGroup = QtGui.QGroupBox(self)
        self.diskGroup.setMinimumSize(QSize(380,100))
        ...
        self.connect(self.manageLvm,QtCore.SIGNAL("clicked()"),parent.manageLvmPartitions)
        self.connect(self.manageRaid,QtCore.SIGNAL("clicked()"),parent.manageRaidPartitions)
    ...
    def addPartition(self,name=None,data=None):
        ...
        #elif data._parted_type == parteddata.lvmPartitionType:
        #    partition.setStyleSheet("background-image:none;")
        #elif data._parted_type == parteddata.raidPartitionType:
        #    partition.setStyleSheet("background-image:none;")
        else:
    ...
    def updateSizes(self,toolBoxWidth):
        ...
        for part in self.partitions:
            ...
            _widget.resize(_size,50)
            if _size <= 16:
                _widget.setMinimumSize(QSize(_size,50))
                _widget.setMaximumSize(QSize(_size,50))
            else:
                _widget.setMaximumSize(QSize(_size,50))
...
class PartEdit(QtGui.QWidget):
    ...
    def updateContent(self):
        ...
        elif part._parted_type == parteddata.lvmPartitionType:
            self.ui.formatType.setEnabled(True)
            self.ui.formatCheck.setChecked(True)
            self.ui.formatCheck.setEnabled(True)
