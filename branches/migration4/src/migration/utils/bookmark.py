...
class Bookmark(object):
    def __init__(self, path):
        ...
        self._document = self.dom.createDocument(None, "bookmarks", None)
    ...
    path = property(lambda p: p.__getPath(), lambda p,f: p.__setPath(f))
...
class FirefoxBookmark(Bookmark):
    def __init__(self, path=None):
        Bookmark.__init__(self, path)
    
    def get(self):
        filename = os.path.join(self.path, "bookmarks.html")
    ...
    def __handleVersion2(self, filepath):
...
class OperaBookmark(Bookmark):
    ...
    def get(self):
        ...
        # open file:
        filename = os.path.join(self.path, "opera6.adr")
        bookmarkfile = open(filename)
        # create main node:
...
class InternetExplorerBookmark(Bookmark):
    def __init__(self, path=None):
        Bookmark.__init__(self, path)
