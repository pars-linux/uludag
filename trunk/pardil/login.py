...
def index():
  p.template = site_config['path'] + 'templates/login.tpl'

def login():
  p.template = site_config['path'] + 'templates/login.tpl'

  if not len(p.form['l_username']):
