...
def install(installFirmwares=True):
    ...
    arch = "arm" if __getKernelARCH() == "arm" else "x86"
    pisitools.insinto("/boot/", "arch/%s/boot/bzImage" % arch, "kernel-%s" % suffix)
...
def installLibcHeaders(excludes=[]):
    ...
    arch = __getKernelARCH()

    make_cmd = "O=%s ARCH=%s INSTALL_HDR_PATH=%s/install" % (headers_tmp, arch, headers_tmp)
