...
\title{Pardus}
\author{Barış Metin, baris@pardus.org.tr}
\institute{
  Pardus Projesi \\
...
\pgfdeclareimage[height=2cm]{pardus-wiki}{pardus-wiki}
...
\frame{
  ...
  \only<1>{
	...
	\item \textbf{bugün}: Pardus 2007.1 (16 Mart)
	\item \textbf{2007 sonu}: Pardus 2008 
	\end{itemize}
...
\frame{
  \frametitle{İmla denetimi}
   ...
   \item ı-I, i-İ harflerinin dönüşümleri yüzünden uygulama hatalarına takılmamak
...
\subsection{Sürüm : Pardus 2007.1}
\frame{
    \frametitle{2007.1 Yenilikleri : Pardus ile hep daha iyiye :)}
        \begin{itemize}
            \item Yeni onlarca uygulama, pek çok uygulamanın daha güncel sürümü (280 güncelleme)
            \item KDE 3.5.6
            \item Daha hızlı bir PiSi (depo güncellemeleri ve arama işlevlerinde \%40'a varan hız artışı)
            \item Yeni dil destekleri (Brezilya Portekizcesi hazır, Fıransızca ve İtalyanca yolda)
            \item Kablosuz ağlar için WPA desteği
        \end{itemize}
}
...
\frame{
  \frametitle{Sanal Ofis : www.pardus.org.tr}
   ...
   \only<2>{
   \begin{itemize}
   \item Wiki \\ \emph{http://www.pardus-wiki.org}
       \begin{itemize}
       \item \pgfuseimage{pardus-wiki}
...
\frame{
  \frametitle{Anında Sonuç : Derleme Çiftliği, İkili Paket Depoları}
   ...
   \only<2>{
	...
	\begin{itemize}
    	\item Şu an için resmi depoda 1384 kaynak paket,
    	\item bu paketlerden üretilmiş 1602 ikili paket içeriyor.	
	\end{itemize}
    \item Geliştirme deposu,
	\begin{itemize}
    	\item Şu an için resmi depoda 1586 kaynak paket,
    	\item bu paketlerden üretilmiş 1838 ikili paket içeriyor.	
	\end{itemize}
    \item Büyük oranda katkıcıların bakımını yaptığı contrib deposu,
    	\begin{itemize}
    	\item Şu anda depoda 470 kaynak paket,	
    	\item bu paketlerden üretilmiş 483 ikili paket içeriyor.	
	\end{itemize}
