...
def load_config_optional(parser, options):
    ...
    # Administrator UID
    if parser.has_option("server", "admin"):
        administrators = parser.get("server", "admin")
        options.admin = []
        for username in administrators.split(","):
            if len(username):
                options.admin.append(username)
    else:
